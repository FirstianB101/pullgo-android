...
class SendTeacherApplyClassroomRequestUseCase @Inject constructor(
    private val repository: ApplyClassroomRepository
){
    operator fun invoke(teacherId: Long, classroomId: Long): Flow<Resource<Boolean>> = flow{
        try{
            emit(Resource.Loading<Boolean>())

            repository.sendTeacherApplyClassroomRequest(teacherId, classroomId)
