...
class GetTeacherLessonsOnMonthUseCase @Inject constructor(
    private val repository: LessonsRepository
){
    operator fun invoke(teacherId: Long): Flow<Resource<List<Lesson>>> = flow{
        try{
            emit(Resource.Loading<List<Lesson>>())

            val lessons = repository.getTeacherLessonsOnMonth(teacherId)
