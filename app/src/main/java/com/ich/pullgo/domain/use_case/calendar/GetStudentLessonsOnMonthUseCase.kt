...
class GetStudentLessonsOnMonthUseCase @Inject constructor(
    private val repository: LessonsRepository
){
    operator fun invoke(studentId: Long): Flow<Resource<List<Lesson>>> = flow{
        try{
            emit(Resource.Loading<List<Lesson>>())

            val lessons = repository.getStudentLessonsOnMonth(studentId)
