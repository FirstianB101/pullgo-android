...
class KickTeacherInClassroomUseCase @Inject constructor(
    private val repository: ManageClassroomRepository
) {
    operator fun invoke(classroomId: Long, teacherId: Long): Flow<Resource<Boolean>> = flow{
        try{
            emit(Resource.Loading<Boolean>())

            repository.kickTeacherFromClassroom(classroomId, teacherId)
