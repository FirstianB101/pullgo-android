...
class GetTeacherLessonsOnDateUseCase @Inject constructor(
    private val repository: LessonsRepository
){
    operator fun invoke(teacherId: Long, date: String): Flow<Resource<List<Lesson>>> = flow{
        try{
            emit(Resource.Loading<List<Lesson>>())

            val lessons = repository.getTeacherLessonsOnDate(teacherId, date)
